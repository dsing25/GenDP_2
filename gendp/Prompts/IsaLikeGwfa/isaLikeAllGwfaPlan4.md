# GWFA ISA-Like Rewrite Plan 4: Final Verification Against Original Plan

## Goal Description

Verify that the entire original plan has been carried out. This plan contains the original plan verbatim below. Walk through every acceptance criterion, every per-magic requirement, and every task — verify each one is satisfied in the current code.

## Design Decision: Controller 31/32 Seam Merge

The original plan (AC-8, t10) says controller 32 should "write first/last intv to s1c for seam merge." The implementation moves this write to magic 31 (writeback) instead of magic 32 (finalize), so that magic 32 can read boundary intvs from s1c without MM lookups. This achieves the same goal (no MM lookups for boundary comparison) but distributes the work differently across magics. This is an intentional design reinterpretation documented in Plan 2a.

## Acceptance Criteria

- AC-1: Every AC from the original plan below is MET
- AC-2: Every per-magic requirement from AC-8 is verified in the code
- AC-3: Every task in the original task breakdown is either completed or has an explicit documented decision for why it was done differently
- AC-4: gwfa_check_correctness.py mode 2 -t 56: 295/295 pass
- AC-5: gendp-isa-reviewer finds no hazards in any in-scope magic

## Task Breakdown

| Task ID | Description | Tag |
|---------|-------------|-----|
| v1 | Walk through AC-1 through AC-9 from original plan. For each, verify the code satisfies it. Document evidence. | analyze |
| v2 | Walk through every per-magic requirement in AC-8. For each, grep/read the code and confirm. | analyze |
| v3 | Walk through original task table t1-t60. For each, confirm it was completed or document the decision. | analyze |
| v4 | Run gendp-isa-reviewer on all controller magics 16-39 | coding |
| v5 | Run gendp-isa-reviewer on all PE magics 19-23 | coding |
| v6 | Fix any issues found | coding |
| v7 | Final verification: mode 2 -t 56 (295/295 required) | coding |

---

## Original Plan (verbatim copy for verification reference)
