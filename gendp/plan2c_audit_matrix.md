...
| 37 | fix-preferred (m37, intv_split_load) | fix-preferred (m37, intv_split_load) | pass (m37 R3 COMPLETE Round 16: bs_lo/bs_hi/bs_tgt → s1c[0..8] Round 12; a_sp/b_sp → s1c[40..49]; tile arrays a0s/a1s/b0s/b1s → s1c[50..65]; source arrays a_srcs/b_srcs → s1c[66..73]; bvd0/bvd1/bvd2 per-PE re-reads from s1c[159..161]; pts[4] per-PE re-reads from s1c[pe]; pe_base → gr[5] (gr[4]=out_buf caller contract preserved per BL-20260421)) | pass (m37 R4/R9 label/goto for while + per-p update branches) | pass | waive:no-spm-read (m37, intv_split_load) | pass (m37 R7 waitLSQ+NOP) | fix-preferred (m37, intv_split_load) | pass (m37 R9 chunk-outer) | waive:no-minmax (m37, intv_split_load) | pass | pass |
