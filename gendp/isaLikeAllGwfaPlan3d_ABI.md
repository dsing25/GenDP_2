...
| gr[1]  | m19 (unused — d_idx moved to reg[1] per Round 5 amendment); m20 lo=i / hi=tile_n; m21 lo=i / hi=tile_n; m22 (unused — moved to reg[1..15, 28..31] per Round 4); m23 (unused — moved to reg[1..15, 28..31] per Round 3) | Per-magic loop bound + index packed lo/hi | Dead on each magic boundary                                                   | NEW       |
| gr[2]  | m19 (unused — vd moved to reg[6] per Round 5); m20 vd0 / vd1 (pair-loaded); m21 vd0 / vd1; m22 (unused — see gr[1] note for Round 4); m23 (unused — see gr[1] note for Round 3, gr is `addr_regfile` so C++ reference aliasing infeasible) | Diag/scatter element scratch | Dead on each boundary | NEW       |
| gr[3]  | m19 (unused — k moved to reg[7] per Round 5); m20 shift; m21 shift; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                                                  | Half-pair load companion to gr[2] for mvd                                                                      | Dead on each boundary                                                         | NEW       |
| gr[4]  | m19 (unused — v/d_val derived inline from vd per Round 5); m20 bin0 / bin1; m21 off0 / off1; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                          | Half-reg extraction target / pair offsets                                                                      | Dead on each boundary                                                         | NEW       |
| gr[5]  | m19 (unused — i_val derived inline from vd+k per Round 5); m20 counts base ptr-offset; m21 bin_spm_off; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                | Per-magic local accumulator                                                                                    | Dead on each boundary                                                         | NEW       |
| gr[6]  | m19 (unused — arc_idx moved to reg[5]; ow moved to reg[12]); m20 (unused); m21 (unused); m22 (free post Round 8 amendment); m23 (unused — see gr[1] note)                                          | Per-magic accumulator                                                                                          | Dead on each boundary                                                         | EXTENDED |
| gr[7]  | m19 (unused — packed_vw moved to reg[11] per Round 5); m20 (scratch); m21 tile_bin_counts ptr; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                        | General scratch                                                                                                | Dead on each boundary                                                         | NEW       |
| gr[8]  | m19 (unused — w derived inline from packed_vw per Round 5); m20 (scratch); m21 (scratch); m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                            | Half-reg pair / emit register                                                                                  | Dead on each boundary                                                         | NEW       |
| gr[9]  | m19 (unused — hkey moved to reg[14] per Round 5); m20 i (loop); m21 i (loop); m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                                          | Full-width per-magic scalar                                                                                    | Dead on each boundary                                                         | NEW       |
...
| gr[11] | m19 (unused — absent moved to reg[15] per Round 5); m20 (unused); m21 (unused); m22 (unused — see gr[1] note for Round 4); m23 (unused — no absent concept in dedup body) | Per-magic small-state scratch                                                                                  | Dead on each boundary                                                         | NEW       |
| gr[12] | m19 (unused — q_char now block-local on reg[28] per Round 5); m20 fin0_base scratch; m21 fin0_base scratch; m22 cum_oi (full int — Round 8 amendment, value >16 bits in practice); m23 (unused — see gr[2] note) | Per-magic scratch / m22 cum_oi cross-call accumulator | Dead on each boundary | EXTENDED |
| gr[13] | m19 (unused — gs_char now block-local on reg[29] per Round 5); m20 (unused); m21 (unused); m22 pe_global_base (full int — Round 8 amendment); m23 (unused — see gr[2] note) | Per-magic scratch / m22 pe_global_base cross-call constant | Dead on each boundary | EXTENDED |
| gr[14] | m19 (unused — n_A moved to reg[2], n_B moved to reg[3] per Round 5); m20 SORT_META offset cache; m21 SORT_META offset cache; m22 (unused); m23 (unused — see gr[2] note) | Output count / per-magic                                                                                        | Dead on each boundary                                                         | NEW       |
| gr[15] | m19 (unused — n_HA moved to reg[4] per Round 5); m20 lo bin0 / hi bin1 packed; m21 lo bin0 / hi bin1 packed; m22 (free); m23 (unused — see gr[2] note) | Per-magic count                                                                                                | Dead on each boundary                                                         | NEW       |
...
| reg[1..15]    | (frozen reference m8/m13 cross-call; m19 + m22 + m23 within-call macro temp + body-local scratch — Round 3 / Round 4 / Round 5 amendment) | reg[1..11] are written by PE 8 (frozen ref pe.cpp:890-891 sets reg[10]=gs_base, reg[11]=q_base; m8_outer_loop body sets reg[1, 2, 6, 7, 8, 9]) and PE 13 (frozen ref pe.cpp:1226-1227 sets reg[10]=gs_base, reg[11]=q_base; m13 body sets reg[1, 6, 7]). DEC-1 cross-call state therefore CANNOT use reg[1..10]; Round 9c relocates DEC-1 state to reg[16..27]. Round 3/4/5 amendment claims reg[1..15] (and reg[28..31]) for m19 + m22 + m23 WITHIN-CALL state. m23 mapping: reg[1]=M23_RI lo / M23_RD vd, reg[2]=M23_RI hi / M23_RD k, reg[3]=M23_PI lo, reg[4]=M23_PI hi, reg[5..8]=M23_PI tc_/tw_/tt_/tb_, reg[9]=p, reg[10]=n_do, reg[11]=n_io, reg[12]=dtn, reg[13]=don_, reg[14]=itn, reg[15]=ion. m22 mapping (Round 4): reg[1]=ai, reg[2]=bi, reg[3]=aw, reg[4]=bw, reg[5]=a_n, reg[6]=b_n, reg[7]=ab, reg[8]=bb, reg[9]=oi, reg[10]=ai0, reg[11]=bi0, reg[12]=out_lo, reg[13]=out_hi, reg[14]=bvd_0, reg[15]=bvd_1. m19 mapping (Round 5): reg[1]=d_idx, reg[2]=n_A, reg[3]=n_B, reg[4]=n_HA, reg[5]=arc_idx, reg[6]=vd, reg[7]=k, reg[8]=nv, reg[9]=n_ext, reg[10]=a_idx, reg[11]=packed_vw, reg[12]=ow, reg[13]=ts_off, reg[14]=hkey, reg[15]=absent. m19 + m22 + m23 SHARE the band — magics fire serially (PE 8 → PE 19 → PE 13 → PE 20 → PE 21 → PE 22 → magic 29 → PE 23), each overwrites these slots at entry. | EXTENDED  |
...
| reg[28..31]   | m19 + m22 + m23 within-call body locals (Round 3 / Round 4 / Round 5 amendment) | m23 mapping: reg[28]=db, reg[29]=ib, reg[30]=ad, reg[31]=ai. m22 mapping: reg[28]=bvd_2, reg[29]=gpos, reg[30]=scratch_lo (head_a_lo / probe_h / o), reg[31]=scratch_hi (head_b_lo / probe_l / on). m19 mapping (Round 5): reg[28]=scratch_a (probe / q_char / nd / sd / id2 / del_vd / bo), reg[29]=scratch_b (h2 / b / gs_char / nvd / svd / ivd), reg[30]=scratch_c (lo / q_phys / gs_phys / q_bp2 / gs_bp2), reg[31]=scratch_d (hi / gs_pos / general scratch). m19 + m22 + m23 SHARE the band — each writes at entry before reading. Round 9c freed this band from prior reg[16..31] PE 21 bin_cursors role (see Section 3.3). | EXTENDED  |
