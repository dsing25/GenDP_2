...
| gr[6]  | m19 lo=arc_idx / hi=ow; m20 (unused); m21 (unused); m22 lo=cum_oi / hi=pe_global_base; m23 db / ib (state-X buffer-base scratch)                       | Per-magic accumulator / packed half pair                                                                       | Dead on each boundary                                                         | NEW       |
...
| gr[12] | m19 q_char (2-bit); m20 fin0_base scratch; m21 fin0_base scratch; m22 b_buf cur_n cache / on switch helper; m23 k scratch                              | 2-bit / single-cycle scratch                                                                                   | Dead on each boundary                                                         | NEW       |
| gr[13] | m19 gs_char (2-bit); m20 (unused); m21 (unused); m22 (unused); m23 lo / hi pair scratch (M23_RI/PI temps)                                              | 2-bit / paired scratch                                                                                         | Dead on each boundary                                                         | NEW       |
