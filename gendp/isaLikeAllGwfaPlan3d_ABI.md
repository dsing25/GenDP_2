...
| gr[1]  | m19 lo=d (diag index) / hi=n_diags; m20 lo=i / hi=tile_n; m21 lo=i / hi=tile_n; m22 (unused — within-call state moved to reg[1..15, 28..31] per Round 4); m23 (unused — within-call state moved to reg[1..15, 28..31] per Round 3) | Per-magic loop bound + index packed lo/hi | Dead on each magic boundary                                                   | NEW       |
| gr[2]  | m19 vd (packed v\|d_biased); m20 vd0 / vd1 (pair-loaded); m21 vd0 / vd1; m22 (unused — see gr[1] note for Round 4); m23 (unused — see gr[1] note for Round 3, gr is `addr_regfile` so C++ reference aliasing infeasible) | Diag/scatter element scratch | Dead on each boundary | NEW       |
| gr[3]  | m19 k (diag k); m20 shift; m21 shift; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                                                    | Half-pair load companion to gr[2] for mvd                                                                      | Dead on each boundary                                                         | NEW       |
| gr[4]  | m19 lo=v / hi=d_val (half-extracted from vd); m20 bin0 / bin1; m21 off0 / off1; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)          | Half-reg extraction target / pair offsets                                                                      | Dead on each boundary                                                         | NEW       |
| gr[5]  | m19 i_val (= d + k); m20 counts base ptr-offset; m21 bin_spm_off; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                          | Per-magic local accumulator                                                                                    | Dead on each boundary                                                         | NEW       |
| gr[6]  | m19 lo=arc_idx / hi=ow; m20 (unused); m21 (unused); m22 (free post Round 8 amendment); m23 (unused — see gr[1] note)                                  | Per-magic accumulator                                                                                          | Dead on each boundary                                                         | EXTENDED |
| gr[7]  | m19 packed_vw scratch; m20 (scratch); m21 tile_bin_counts ptr; m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                            | General scratch                                                                                                | Dead on each boundary                                                         | NEW       |
| gr[8]  | m19 lo=w (vertex index hi-half) / hi=hkey-hi; m20 (scratch); m21 (scratch); m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)              | Half-reg pair / emit register                                                                                  | Dead on each boundary                                                         | NEW       |
| gr[9]  | m19 hkey (full 32-bit packed v\|i+1); m20 i (loop); m21 i (loop); m22 (unused — see gr[1] note); m23 (unused — see gr[1] note)                          | Full-width per-magic scalar                                                                                    | Dead on each boundary                                                         | NEW       |
...
| gr[11] | m19 absent (–1/0/1); m20 (unused); m21 (unused); m22 (unused — see gr[1] note for Round 4); m23 (unused — no absent concept in dedup body)              | Per-magic small-state scratch                                                                                  | Dead on each boundary                                                         | NEW       |
| gr[12] | m19 q_char (2-bit); m20 fin0_base scratch; m21 fin0_base scratch; m22 cum_oi (full int — Round 8 amendment, value >16 bits in practice); m23 (unused — see gr[2] note) | Per-magic scratch / m22 cum_oi cross-call accumulator | Dead on each boundary | EXTENDED |
| gr[13] | m19 gs_char (2-bit); m20 (unused); m21 (unused); m22 pe_global_base (full int — Round 8 amendment); m23 (unused — see gr[2] note) | Per-magic scratch / m22 pe_global_base cross-call constant | Dead on each boundary | EXTENDED |
| gr[14] | m19 lo=n_A / hi=n_B; m20 SORT_META offset cache; m21 SORT_META offset cache; m22 (unused); m23 (unused — see gr[2] note)                                | Output count / per-magic                                                                                        | Dead on each boundary                                                         | NEW       |
| gr[15] | m19 n_HA; m20 lo bin0 / hi bin1 packed; m21 lo bin0 / hi bin1 packed; m22 (free); m23 (unused — see gr[2] note)                                         | Per-magic count                                                                                                | Dead on each boundary                                                         | NEW       |
...
| reg[1..15]    | (frozen reference m8/m13 cross-call; m22 + m23 within-call macro temp + body-local scratch — Round 3 / Round 4 amendment) | reg[1..11] are written by PE 8 (frozen ref pe.cpp:890-891 sets reg[10]=gs_base, reg[11]=q_base; m8_outer_loop body sets reg[1, 2, 6, 7, 8, 9]) and PE 13 (frozen ref pe.cpp:1226-1227 sets reg[10]=gs_base, reg[11]=q_base; m13 body sets reg[1, 6, 7]). DEC-1 cross-call state therefore CANNOT use reg[1..10]; Round 9c relocates DEC-1 state to reg[16..27]. Round 3/4 amendment claims reg[1..15] (and reg[28..31]) for m22 + m23 WITHIN-CALL state. m23 mapping: reg[1]=M23_RI lo / M23_RD vd, reg[2]=M23_RI hi / M23_RD k, reg[3]=M23_PI lo, reg[4]=M23_PI hi, reg[5..8]=M23_PI tc_/tw_/tt_/tb_, reg[9]=p, reg[10]=n_do, reg[11]=n_io, reg[12]=dtn, reg[13]=don_, reg[14]=itn, reg[15]=ion. m22 mapping (Round 4): reg[1]=ai, reg[2]=bi, reg[3]=aw, reg[4]=bw, reg[5]=a_n, reg[6]=b_n, reg[7]=ab, reg[8]=bb, reg[9]=oi, reg[10]=ai0, reg[11]=bi0, reg[12]=out_lo, reg[13]=out_hi, reg[14]=bvd_0, reg[15]=bvd_1. m22 and m23 SHARE the band — magics fire serially (sort → merge → magic 29 → dedup), each overwrites these slots at entry. | EXTENDED  |
...
| reg[28..31]   | m22 + m23 within-call body locals (Round 3 / Round 4 amendment) | m23 mapping: reg[28]=db, reg[29]=ib, reg[30]=ad, reg[31]=ai. m22 mapping: reg[28]=bvd_2, reg[29]=gpos, reg[30]=scratch_lo (head_a_lo / probe_h / o), reg[31]=scratch_hi (head_b_lo / probe_l / on). m22 and m23 SHARE the band — each writes at entry before reading. Round 9c freed this band from prior reg[16..31] PE 21 bin_cursors role (see Section 3.3). | EXTENDED  |
